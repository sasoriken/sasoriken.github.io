...
私自身の技術力向上のためには、専門分野の論文を日常的に読むべきと思っていたのですが、多趣味であるため、論文を読むことにあまり時間をかけたくないという思いがありました。
ここに更に英語が加わるとモチベーションは皆無です。そこでふとした時に生成AIに論文の概略をまとめさせる有用性に気づき、それならとこの作業を全自動化＆生成AIにキャラクターを持たせてユーモアを追加したら、論文を読むためのモチベーション維持に繋がると思い実装しました。
...
本業では GitHub Actions についてあまり触れておらず仕様理解に少し苦労しました。GITHUB上のUIでも非常に分かりにくく反面教師にしようと思いました。
それ以外ではキャラクターの一貫性を保つことに試行錯誤しました。アーキテクチャの特性上、ＡＩの記憶の保持はしていないため、そのうえで一貫したキャラクターを持たせる必要があり現在も試行錯誤中です。
